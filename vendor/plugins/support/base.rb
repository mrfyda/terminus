...
unless defined?(::Rails)
  module ::Rails # rubocop:disable Style/ClassAndModuleChildren
    ...
    # Sidekiq's CLI checks `Rails.respond_to?(:application)` to decide
    # whether to log a "Booted Rails X.Y.Z application..." banner, then
    # calls this unconditionally. Without it, the stub above makes Sidekiq
    # think it's running under Rails and crashes on boot with a
    # NoMethodError before processing a single job.
